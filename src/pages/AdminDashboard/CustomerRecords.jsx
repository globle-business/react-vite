...
export default function CustomerRecords() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (selectedCustomer) {
        await API.put(`/user/update/${selectedCustomer.id}`, {
    ...
    <div className="min-h-full p-8 text-white bg-gradient-to-br from-[#0f172a] via-[#111827] to-[#1e293b] space-y-8">
      ...
      <div className="bg-gray-900/70 border border-gray-700 rounded-xl overflow-x-auto">
        <table className="min-w-full text-left">
          ...
          <tbody>
            {customers.map((cust) => (
              <tr key={cust.id} className="border-b border-gray-800">
                ...
                <td className="p-4 flex justify-center gap-4">
                  <button onClick={() => handleView(cust.id)} className="text-sky-400">
                    <Eye size={18} />
                  </button>

                  <button onClick={() => openEditModal(cust)} className="text-yellow-400">
                    <Edit size={18} />
                  </button>
